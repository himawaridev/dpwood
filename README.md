...
CI tại [`.github/workflows/ci.yml`](.github/workflows/ci.yml) chạy test backend, lint và build frontend. Với thay đổi thanh toán, auth, tồn kho hoặc email, vẫn cần kiểm tra API và luồng giao diện trên database test trước khi deploy.

## Migration, backup và phục hồi

Chạy migration trước khi khởi động phiên bản backend mới:
...
Tạo logical backup JSON có dấu thời gian:

```powershell
pnpm db:backup
```

Khôi phục chỉ nên thực hiện trong thời gian bảo trì và sau khi đã tạo thêm một bản backup mới:

```powershell
pnpm db:restore -- --file=C:\backup\dpwood.json --confirm
```

Migration có lịch sử trong bảng `SchemaMigrations`, có thể chạy lặp lại mà không thêm trùng cột hoặc index. Xem quy trình vận hành tại [docs/COMMERCE_OPERATIONS.md](docs/COMMERCE_OPERATIONS.md).
...
- Chạy `pnpm db:migrate` trong bước release trước khi nhận traffic.
- Chạy `pnpm products:quality` sau import catalog để phát hiện SKU, ảnh và dữ liệu Merchant Center chưa đạt.
...
- Bật 2FA cho tài khoản `admin`/`root`, kiểm tra và thu hồi phiên đăng nhập không còn sử dụng.
- Theo dõi `/api/analytics/metrics` và dashboard quản trị để phát hiện tỷ lệ lỗi hoặc thời gian phản hồi tăng.
...
- [Vận hành commerce](docs/COMMERCE_OPERATIONS.md)
- [Google Merchant Center](docs/MERCHANT_CENTER.md)
- [Checklist bảo mật](docs/SECURITY_ASVS_CHECKLIST.md)
